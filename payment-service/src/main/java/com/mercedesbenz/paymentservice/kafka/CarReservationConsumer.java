...
public class CarReservationConsumer {
    ...
    @KafkaListener(topics = "{topic.name.consumer.cars}", groupId = "${spring.kafka.consumer.group-id}")
    public void consume(ConsumerRecord<String, ReservationDto> payload) {
        ...
        paymentService.addFlightReservation(payload.value().getId(), ReservationType.CAR);
    }
}
